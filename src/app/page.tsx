...
export default function Dashboard() {
  ...
  const fetchUsers = async (ownerId?: string) => {
    ...
    if (!oId) return;
    try {
      const url = `/api/users?ownerId=${oId}&t=${Date.now()}`;
  ...
  async function fetchAttendance() {
    try {
      ...
      const url = `/api/attendance?date=${today}&ownerId=${oId}&t=${Date.now()}`;
      const res = await fetch(url);
      const data = await res.json();
      if (data.success) {
  ...
  async function fetchShiftSwapRequests() {
    try {
      ...
      const oId = activeUser?.ownerId || activeUser?.id;
      if (!oId) return;
      const url = `/api/shift-swap?ownerId=${oId}&t=${Date.now()}`;
      const res = await fetch(url);
      const data = await res.json();
      if (!data.error) {
  ...
  async function fetchLeaveRequests() {
    try {
      ...
      const oId = activeUser?.ownerId || activeUser?.id;
      if (!oId) return;
      const url = `/api/leave?ownerId=${oId}&t=${Date.now()}`;
      const res = await fetch(url);
      const data = await res.json();
      if (!data.error) {
          ...
          <section style={{ padding: "40px 32px", overflowY: "auto", flexGrow: 1, minHeight: 0 }}>
            <div className="glass-card" style={{ padding: "32px", display: "flex", flexDirection: "column", minHeight: "min-content" }}>
              {/* ========== STAFF ATTENDANCE SUB-TAB ========== */}
                  ...
                  {((currentUser?.role === "Super Admin" || hasPermission("attendance:manual-clock") || hasPermission("attendance:approve-leave") || hasPermission("attendance:approve-swap")) ? usersList : usersList.filter(u => u.id === currentUser?.id)).length === 0 ? (
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: "1fr", gap: "12px" }}>
                      {((currentUser?.role === "Super Admin" || hasPermission("attendance:manual-clock") || hasPermission("attendance:approve-leave") || hasPermission("attendance:approve-swap")) ? usersList : usersList.filter(u => u.id === currentUser?.id)).map((user: any) => {
                    ...
                    <div key={user.id} className="glass-card" style={{ padding: "12px 16px", display: "flex", alignItems: "center", justifyContent: "space-between", gap: "12px", flexWrap: "wrap" }}>
                      ...
                      <div style={{ display: "flex", gap: "8px", alignItems: "center", flexShrink: 0 }}>
                        ...
                        {editingShiftUserId === user.id ? (
                          ...
                          (currentUser?.role === "Super Admin" || hasPermission("staff-management:edit-shift")) && (
                            <>
                              <button
                                onClick={() => {
