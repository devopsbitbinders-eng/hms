...
export default function FrontDeskOps({
  ...
  const [checkoutCouponData, setCheckoutCouponData] = useState<any>(null);
  ...
  const handleCheckout = async () => {
    ...
    if (checkoutCouponCode && !checkoutCouponData) {
      addToast("Please apply the coupon before checking out.", "warning");
  ...
  const handleApplyCoupon = async () => {
    if (!checkoutRes || !checkoutCouponCode.trim()) return;
    ...
    try {
      ...
      if (calcData.success) {
        setCheckoutCouponData(calcData.data);
        addToast(`✅ Coupon ${checkoutCouponCode} applied! Discount: ₹${calcData.data.discountAmount.toFixed(2)}`, "success");
      } else {
  ...
  const handleRemoveCoupon = async () => {
    if (!checkoutRes) return;
    try {
      const res = await fetch(`/api/billing/coupon?reservationId=${checkoutRes.id}`, { method: "DELETE" });
      const data = await res.json();
      if (data.success) {
        setCheckoutCouponCode("");
        setCheckoutCouponData(null);
  ...
  const handleOpenCheckout = (res: Reservation) => {
    setCheckoutRes(res);
    setCheckoutCouponCode(res.couponId || ""); // Optional
    setCheckoutCouponData(null);
  };
            ...
            <div style={{ padding: "24px", maxHeight: "75vh", overflowY: "auto" }}>
                ...
                return (
                  <div style={{ background: "rgba(0,0,0,0.3)", borderRadius: "10px", padding: "16px", marginBottom: "20px" }}>
                    ...
                    <div style={{ borderTop: "1px solid rgba(255,255,255,0.1)", marginTop: "10px", paddingTop: "10px" }}>
                      ...
                      {checkoutCouponData && checkoutCouponData.discountAmount > 0 && (
                        <div style={{ display: "flex", justifyContent: "space-between", fontSize: "0.82rem", color: "#10b981", marginBottom: "10px", fontWeight: "600" }}>
                          <span>Discount ({checkoutCouponData.couponApplied})</span>
                          <span>- {formatCurrency(checkoutCouponData.discountAmount)}</span>
                        </div>
                      )}
                      
                      {checkoutCouponData ? (
                        <>
                          <div style={{ display: "flex", justifyContent: "space-between", fontWeight: "700", fontSize: "1.1rem", color: "#ec4899" }}>
                            <span>Total Due</span><span>{formatCurrency(checkoutCouponData.grandTotal)}</span>
                          </div>
                          <div style={{ fontSize: "0.7rem", color: "var(--text-secondary)", textAlign: "right", marginTop: "2px", textDecoration: "line-through" }}>
                            Original: {formatCurrency(bill.total)}
                          </div>
                        </>
                      ) : (
              ...
              <div style={{ marginBottom: "20px", display: "flex", gap: "10px" }}>
                <input
                  ...
                  disabled={checkoutCouponData !== null}
                />
                {checkoutCouponData ? (
                  <button className="btn-secondary" onClick={handleRemoveCoupon}>Remove</button>
                ) : (
                  <button className="btn-primary" onClick={handleApplyCoupon} disabled={isApplyingCoupon || !checkoutCouponCode.trim()}>
